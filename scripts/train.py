...
def save_checkpoint(model, config, epoch, iteration, num_trial=10):
    ...
    for i in range(num_trial):
        try:
            ...
            torch.save(config, op.join(checkpoint_dir, 'training_args.bin'))
...
def run(config, train_dataloader, Graphormer_model, mano_model, renderer, mesh_sampler):
    ...
    criterion_2d_keypoints = torch.nn.MSELoss(reduction='none').cuda(config['exper']['device'])
    criterion_keypoints = torch.nn.MSELoss(reduction='none').cuda(config['exper']['device'])
    criterion_vertices = torch.nn.L1Loss().cuda(config['exper']['device'])

    if config['exper']['distribute']:
    ...
    for iteration, (img_keys, images, annotations) in enumerate(train_dataloader):
        ...
        loss_vertices = (
                    config['exper']['loss']['vloss_w_sub'] * vertices_loss(criterion_vertices, pred_vertices_sub, gt_vertices_sub, has_mesh) + \
                    config['exper']['loss']['vloss_w_full'] * vertices_loss(criterion_vertices, pred_vertices, gt_vertices, has_mesh))

        # compute 3d joint loss (where the joints are regressed from full mesh)
        loss_reg_3d_joints = keypoint_3d_loss(criterion_keypoints, pred_3d_joints_from_mesh, gt_3d_joints_with_tag,
                                              has_3d_joints)
        ...
        loss = config['exper']['loss']['joints_loss_weight'] * loss_3d_joints + \
               config['exper']['loss']['vertices_loss_weight'] * loss_vertices + config['exper']['loss']['vertices_loss_weight'] * loss_2d_joints
        ...
        if iteration % config['utils']['logging_steps'] == 0 or iteration == max_iter:
            ...
            if is_main_process() == True:
                ...
                temp_fname = config['exper']['output_dir'] + 'visual_' + stamp + '.jpg'
                cv2.imwrite(temp_fname, np.asarray(visual_imgs[:, :, ::-1] * 255))
                # aml_run.log_image(name='visual results', path=temp_fname)

        if iteration % iters_per_epoch == 0:
            if epoch % 10 == 0:
                checkpoint_dir = save_checkpoint(Graphormer_model, config, epoch, iteration)
...
#     #########################################################
#     parser.add_argument("--model_name_or_path", default='src/modeling/bert/bert-base-uncased/', type=str,
#                         required=False,
...
def main(config):
    ...
    logger = setup_logger("Graphormer", config['exper']['output_dir'], get_rank())
    set_seed(config['exper']['seed'], num_gpus)
    logger.info("Using {} GPUs".format(num_gpus))

    # Mesh and SMPL utils
    mano_model = MANO().to(config['exper']['device'])
    ...
    input_feat_dim = config['model']['tfm']['input_feat_dim']
    hidden_feat_dim = config['model']['tfm']['hidden_feat_dim']
    output_feat_dim = input_feat_dim[1:] + [3]

    # which encoder block to have graph convs
    which_blk_graph = config['model']['tfm']['which_gcn']
    ...
    if config['exper']['run_eval_only'] == True and config['exper']['resume_checkpoint'] != None and\
        ...
        logger.info("Evaluation: Loading from checkpoint {}".format(config['exper']['resume_checkpoint']))
        ...
        for i in range(len(output_feat_dim)):
            ...
            b_config = config_class.from_pretrained(config['model']['bert_config'])

            b_config.output_attentions = config['model']['bert']['output_attentions']
            b_config.hidden_dropout_prob = config['model']['bert']['drop_out']
            ...
            b_config.mesh_type = config['model']['tfm']['mesh_type']
            b_config.num_hidden_layers = config['model']['tfm']['num_hidden_layers']
            b_config.num_attention_heads = config['model']['tfm']['num_attention_heads']
            ...
            if config['model']['backbone']['arch'] == 'hrnet':
                hrnet_update_config(hrnet_config, config['model']['backbone']['hrnet_yaml'])
                backbone = get_cls_net_gridfeat(hrnet_config, pretrained=config['model']['backbone']['hrnet_bb'])
                logger.info('=> loading hrnet model')

            else:
                print("=> using pre-trained model '{}'".format(config['model']['backbone']['arch']))
                backbone = models.__dict__[config['model']['backbone']['arch']](pretrained=True)
        ...
        if config['exper']['resume_checkpoint'] != None and config['exper']['resume_checkpoint'] != 'None':
            ...
            logger.info("Loading state dict from checkpoint {}".format(config['exper']['resume_checkpoint']))
    ...
    logger.info("Training parameters %s", str(config))

    if config['exper']['run_eval_only'] == True:
        val_dataloader = make_hand_data_loader(args, args.val_yaml,
                                               config['exper']['distributed'], is_train=False, scale_factor=config['data']['img_scale_factor'])
        run_eval_and_save(args, 'freihand', val_dataloader, _model, mano_model, renderer, mesh_sampler)

    else:
        train_dataloader = make_hand_data_loader(config, args.train_yaml,
                                                 config['exper']['distributed'], is_train=True, scale_factor=config['data']['img_scale_factor'])
