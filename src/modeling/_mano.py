...
from smplx import MANO as MANOOrigin


class MANO(MANOOrigin):
    def __init__(self, smplx_path, use_pca=False, is_rhand=True):
        super(MANO, self).__init__(smplx_path, use_pca=use_pca, is_rhand=is_rhand)
        self.joint_regressor = self.J_regressor.numpy()
        # add fingertips to joint_regressor
        self.fingertip_vertex_idx = [744, 320, 443, 554, 671]  # mesh vertex idx (right hand)
        thumbtip_onehot = np.array([1 if i == 744 else 0 for i in range(self.joint_regressor.shape[1])],
                                   dtype=np.float32).reshape(1, -1)
        indextip_onehot = np.array([1 if i == 320 else 0 for i in range(self.joint_regressor.shape[1])],
                                   dtype=np.float32).reshape(1, -1)
        middletip_onehot = np.array([1 if i == 443 else 0 for i in range(self.joint_regressor.shape[1])],
                                    dtype=np.float32).reshape(1, -1)
        ringtip_onehot = np.array([1 if i == 554 else 0 for i in range(self.joint_regressor.shape[1])],
                                  dtype=np.float32).reshape(1, -1)
        pinkytip_onehot = np.array([1 if i == 671 else 0 for i in range(self.joint_regressor.shape[1])],
                                   dtype=np.float32).reshape(1, -1)
...
#
#         self.mano_dir = '/userhome/alanjjp/Project/MeshGraphormer/src/modeling/data'
