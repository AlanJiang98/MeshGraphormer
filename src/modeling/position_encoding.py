...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, bs, hws, device):
        h_all, w_all = 0, 0
        h_t, w_t = 0, 0
        pos_list = []
        for hw in hws:
            ...
            w_all += hw[1]
        for hw in hws:
            ones = torch.ones((bs, hw[0], hw[1]), dtype=torch.bool, device=device)
            y_embed = ones.cumsum(1, dtype=torch.float32) + h_t
            x_embed = ones.cumsum(2, dtype=torch.float32) + w_t
            if self.normalize:
                eps = 1e-6
                y_embed = y_embed / (h_all + eps) * self.scale
                x_embed = x_embed / (w_all + eps) * self.scale
            ...
            pos = torch.cat((pos_y, pos_x), dim=3).permute(0, 3, 1, 2).flatten(2)
            pos_list.append(pos)
            h_t += hw[0]
            w_t += hw[1]
        pos_all = torch.cat(pos_list, dim=2)
        return pos_all
