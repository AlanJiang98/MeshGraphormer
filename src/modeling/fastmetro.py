...
class FastMETRO_Body_Network(nn.Module):
    ...
    def __init__(self, args, backbone, mesh_sampler, num_joints=14, num_vertices=431):
        ...
        adjacency_indices = torch.load('./fastmetro/modeling/data/smpl_431_adjmat_indices.pt')
        adjacency_matrix_value = torch.load('./fastmetro/modeling/data/smpl_431_adjmat_values.pt')
        adjacency_matrix_size = torch.load('./fastmetro/modeling/data/smpl_431_adjmat_size.pt')
    ...
    def forward(self, images):
        ...
        img_features = self.conv_1x1(img_features).flatten(2).permute(2, 0, 1)

        # positional encodings
        pos_enc_1 = self.position_encoding_1(batch_size, h, w, device).flatten(2).permute(2, 0, 1)
        pos_enc_2 = self.position_encoding_2(batch_size, h, w, device).flatten(2).permute(2, 0, 1)

        # first transformer encoder-decoder
        cam_features_1, enc_img_features_1, jv_features_1 = self.transformer_1(img_features, cam_token, jv_tokens,
                                                                               pos_enc_1, attention_mask=attention_mask)
        ...
        reduced_jv_features_1 = self.dim_reduce_dec(jv_features_1)

        # second transformer encoder-decoder
        cam_features_2, _, jv_features_2 = self.transformer_2(reduced_enc_img_features_1, reduced_cam_features_1,
                                                              reduced_jv_features_1, pos_enc_2,
                                                              attention_mask=attention_mask)

        # estimators
        cam_parameter = self.cam_predictor(cam_features_2).view(batch_size, 3)
        pred_3d_coordinates = self.xyz_regressor(jv_features_2.transpose(0, 1))
